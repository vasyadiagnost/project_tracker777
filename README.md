...
Для первичного релиза удобно сделать так:

1. загрузить исходники в репозиторий;
2. локально запустить `build_exe.bat`;
3. открыть GitHub → **Releases** → **Draft a new release**;
4. создать тег, например `v1.0.0`;
5. приложить `dist/ProjectTracker.exe` как release asset;
6. в описании релиза кратко перечислить возможности приложения.

Пример заголовка релиза:

```text
Project Tracker Desktop v1.0.0
```
...
- проверка цветовых сигналов на главной;
- генерация сводки;
- экспорт TXT/XLSX/PDF;
- сборка через `build_exe.bat`;
- запуск `dist/ProjectTracker.exe`.
